...
export function resolveConfig(options: PluginOptions): Config {
  ...
  return {
    ...
    checkpoint:
      stringOption(options, "checkpoint") ?? "fastino/gliner2.5-small-v1",
...
function manifestLines(manifest: Manifest): string[] {
  const lines = [
    `gliner25-pruning: ${Math.round(manifest.reduction * 100)}% reduction ` +
      `(${manifest.characters_before} → ${manifest.characters_after} chars)`,
  ];
  for (const decision of manifest.decisions) {
    lines.push(
      `gliner25-pruning: ${decision.tool_use_id} ${decision.action} ` +
...
async function fallback(
  ...
  if (config.fallback === "unchanged") {
    return { skip: `GLiNER pruning skipped: ${reason}` };
  }
  return next(event);
}
  ...
  on("session.compact", async ($, event, next) => {
    try {
      const configuredPath =
        config.pythonPath ?? (await $.env.get("GLINER_PRUNING_PYTHON"));
      const pythonPath = configuredPath ?? "python3";
      const capturePath = await $.env.get("GLINER_PRUNING_CAPTURE_PATH");
      ...
      const summary =
        `GLiNER pruning: ${Math.round(response.manifest.reduction * 100)}% fewer chars ` +
      ...
      if (response.manifest.reduction < config.minReductionRatio) {
        ...
        return fallback(
          ...
          event,
          next,
          "reduction below configured minimum",
        );
      }
      $.ui.toast(
      ...
      $.ui.log(`gliner25-pruning fallback: ${reason}`);
      $.ui.toast(
        `GLiNER pruning failed, using fallback: ${reason.slice(0, 120)}`,
        {
          timeoutMs: 15_000,
        },
      );
